import { EntityOrId, RelationConfig, Repository, ResourceTransformer } from '@/types/db/relation-resolver.types';
...
export async function resolveRelations<T extends Record<string, any>>(
  ...
  const resolvedRelations = await Promise.all(
    relations.map(async (relation) => {
      ...
      const resolved = await resolveRelation(value, relation.repository, relation.transformer);
