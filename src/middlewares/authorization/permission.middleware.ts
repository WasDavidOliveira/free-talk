...
export const hasPermission = (permissionName: string, action: PermissionAction) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const permission = await db.query.permissions.findFirst({
        where: and(eq(permissions.name, permissionName), eq(permissions.action, action)),
      });

      if (!permission) {
        throw new ForbiddenError(`Permissão ${permissionName}:${action} não encontrada no sistema`);
      }

      const userRoleIds = userData.userRoles.map((ur) => ur.roleId);

      const hasPermission = await db.query.rolePermissions.findFirst({
        where: and(inArray(rolePermissions.roleId, userRoleIds), eq(rolePermissions.permissionId, permission.id)),
      });

      if (!hasPermission) {
        throw new ForbiddenError('Usuário não tem permissão para realizar esta ação');
      }
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (allPermissions.length !== permissionChecks.length) {
        ...
        const missingPermissions = requestedPermissions.filter((rp) => !foundPermissions.includes(rp));

        throw new ForbiddenError(`Permissões não encontradas no sistema: ${missingPermissions.join(', ')}`);
      }

      const permissionIds = allPermissions.map((p) => p.id);
      const userRolePermissions = await db.query.rolePermissions.findMany({
        where: and(inArray(rolePermissions.roleId, userRoleIds), inArray(rolePermissions.permissionId, permissionIds)),
      });
      ...
      if (userRolePermissions.length !== permissionChecks.length) {
        ...
        throw new ForbiddenError(`Usuário não possui as seguintes permissões: ${missingPermissions.join(', ')}`);
      }
