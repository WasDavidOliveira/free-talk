...
export class MessageService {
  ...
  async deleteMessage(
    ...
    const canDelete =
      message.senderId === userId || conversation?.createdBy === userId;
  ...
  async markMessagesAsRead(
    userId: number,
    conversationId: number,
    markAsReadData: MarkAsReadInput
  ) {
    const hasAccess = await this.verifyConversationAccess(
      userId,
      conversationId
    ...
    for (const messageId of messageIds) {
      ...
      if (!message) {
        throw new BadRequestError(
          `Mensagem ${messageId} não encontrada nesta conversa`
        );
      }
    }
  ...
  async getUnreadCount(userId: number, conversationId: number) {
    ...
    const count = await MessageRepository.getUnreadMessagesCount(
      conversationId,
      userId
    );
  ...
  protected async verifyConversationAccess(
    userId: number,
    conversationId: number
  ): Promise<boolean> {
